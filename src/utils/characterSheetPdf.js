...
export async function downloadCharacterSheetPdf({ character, computed, stats, skills, weaponTypes }) {
  ...
  section('Stats', 'strength'); table(['', 'Stat', 'Score'], stats.map(([key, label]) => ['', label, signedEntry(character.stats[key])]), { 0: { cellWidth: 28 }, 2: { cellWidth: 70 } }, { didDrawCell: data => { if (data.section === 'body' && data.column.index === 0) drawIcon(doc, stats[data.row.index][0], data.cell.x + 14, data.cell.y + (data.cell.height / 2)) } })
