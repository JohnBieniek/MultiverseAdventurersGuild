...
function SectionTitle({ title, subtitle }) {
  ...
  return <div className="section-title"><h2 title={startingArray || undefined}>{guideLink ? <a className="section-guide-link" href={guideLink}>{heading}</a> : heading}</h2>{(startingArray || note) && <p className={!note ? 'starting-array-only' : undefined}>{startingArray && <span className="starting-array-note">{startingArray} </span>}{note}</p>}{title === 'Talents' && subtitle && <strong className="section-metric">{subtitle}</strong>}</div>
}
