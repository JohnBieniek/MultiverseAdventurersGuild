...
const expandItemCandidates = candidates => candidates.flatMap(candidate => [
  candidate,
  ...selectionsForName(candidate[0], multiverseItemModels, 9).map((model, variationIndex) => {
    const name = `${model} ${candidate[0]}`
    return [name, uniqueItemDescription(candidate[1], variationIndex)]
  }),
])
...
function CharacterSheet() {
  ...
  return <div className="sheet-page">
    ...
    <header className="sheet-header"><img src="/multiverse%20adventurers%20guild%20icon.png" alt="Guild shield"/><div><span className="eyebrow sheet-eyebrow">MULTIVERSE ADVENTURERS GUILD</span><h1>Character Sheet</h1></div><div className="identity-fields">
      <Field label="Hero name" value={character.name} onChange={setCharacterName} wide/><IdentityChoice label="Species" value={character.species} options={speciesNames} onChange={setSpecies}/><IdentityChoice label="Archetype" value={character.archetype} options={archetypeOptions.map(option => option.name)} tooltip={archetypeOptions.find(option => option.name === character.archetype)?.description} onChange={chooseArchetype} allowReselect/><div className="identity-utility-row"><Field label="Level" type="number" min="0" max="10" value={character.level} onChange={setLevel}/><Field label="Total XP" type="number" min="0" value={character.totalXp} onChange={setTotalXp}/><Field label="Unspent XP" type="number" min="0" value={character.unspentXp} onChange={setUnspentXp}/><div className="quick-dice" aria-label="Quick dice rolls"><span>Roll a Die</span>{[4,6,8,10,20].map(sides=><button type="button" key={sides} onClick={()=>quickDieRoll(sides)}>d{sides}</button>)}</div></div></div></header>
...
function IdentityChoice({ label, value, options, tooltip = '', onChange, allowReselect = false }) { const existing = options.includes(value); const [custom, setCustom] = useState(Boolean(value) && !existing); useEffect(() => { if (existing) setCustom(false) }, [existing]); const choose = event => { if (event.target.value === '__custom__') { onChange(''); setCustom(true) } else onChange(event.target.value) }; const option = name => <option value={name} key={name}>{name}</option>; const selectedValue = allowReselect && existing ? '__current__' : existing ? value : ''; return <label className="field identity-choice" data-tooltip={tooltip}><span>{label}</span>{custom ? <div className="identity-custom"><input autoFocus aria-label={`Custom ${label}`} value={value} placeholder={`Enter custom ${label.toLowerCase()}`} onChange={e => onChange(e.target.value)}/><select className="custom-list-trigger" aria-label={`Choose ${label} from list`} value="" onChange={choose}><option value="" disabled></option>{options.map(option)}</select></div> : <select aria-label={`Choose ${label}`} value={selectedValue} onChange={choose}>{allowReselect && existing && <option value="__current__" hidden>{value}</option>}<option value="" disabled>Choose</option><option value="__custom__">Custom {label.toLowerCase()}…</option>{options.map(option)}</select>}</label> }
