...
const stats = [
  ...
  ['endurance', 'Endurance', 'END'], ['intuition', 'Intuition', 'INT'],
  ['education', 'Education', 'EDU'], ['charisma', 'Charisma', 'CHA'],
]
const skillDefs = [
  ['athletics', 'Athletics', 'endurance'], ['influence', 'Influence', 'charisma'],
  ['knowledge', 'Knowledge', 'education'], ['observation', 'Observation', 'intuition'],
  ['outdoors', 'Outdoors', 'intuition'], ['sneak', 'Sneak', 'dexterity'],
...
function CharacterSheet() {
  ...
  return <div className="sheet-page">
    ...
    <section className="sheet-section vitals"><SectionTitle icon="⚔" title="Combat Summary" subtitle="Move 30 feet each turn. One reaction per round."/><div className="vital-grid">
      <Vital label="Initiative" value={signed(computed.initiative)} roll={() => checkRoll('Initiative', computed.initiative)}/><Vital label="HP" editable value={character.currentHp} max={computed.maxHp} onChange={v => update(['currentHp'], v)}/><Vital label="Temp HP" editable value={character.temporaryHp} onChange={v => update(['temporaryHp'], v)}/><Vital label="Defense" value={computed.defense}/><Vital label="Resilience" value={signed(computed.resilience)} roll={() => checkRoll('Resilience', computed.resilience)}/><Vital label="Ego" value={signed(computed.ego)} roll={() => checkRoll('Ego', computed.ego)}/><Vital label="Energy" editable value={character.currentEnergy} max={computed.maxEnergy} onChange={v => update(['currentEnergy'], v)}/><Vital label="Max Force" value={computed.maxForce}/><Vital label="Talent Slots" value={computed.slots}/></div>
    </section>

    <div className="sheet-columns"><section className="sheet-section"><SectionTitle icon="▥" title="Stats" subtitle="Starting array: +3, +2, +1, 0, 0, −1. Each choice can only be used once, except 0 twice."/><div className="stat-list">{stats.map(([key, label, short]) => <div className="stat-row" key={key}><div><strong>{label}</strong><span>{short}</span></div><ScoreControl label={`${label} score`} value={character.stats[key]} options={[-1, 0, 1, 2, 3]} isOptionDisabled={option => statOptionUnavailable(key, option)} onChange={v => update(['stats', key], v)}/><button className="roll-button" onClick={() => checkRoll(label, character.stats[key])}>Roll</button></div>)}</div></section>
      <section className="sheet-section skills"><SectionTitle icon="★" title="Skills" subtitle="Starting array: +2, +2, +1, +1, +1, 0, 0, 0, −1"/><div className="skill-head"><span>Skill</span><span>Stat</span><span>Ability</span><span>Mod</span><span>Buff</span><span>Debuff</span><span>Total</span><span></span></div>{skillDefs.map(([key, label, defaultStat]) => { const total = skillTotal(key, defaultStat); const statShort = stats.find(([statKey]) => statKey === defaultStat)?.[2]; return <div className="skill-row" key={key}><strong>{label}</strong><span className="skill-stat">{statShort}</span><ScoreControl label={`${label} ability`} value={character.skills[key].ability} options={[-1, 0, 1, 2]} onChange={v => update(['skills', key, 'ability'], v)}/>{['modifier','buffs','debuffs'].map(field => <NumberInput key={field} value={character.skills[key][field]} onChange={v => update(['skills', key, field], field === 'debuffs' ? -Math.abs(number(v)) : v)}/>)}<output>{signed(total)}</output><button className="roll-button" onClick={() => checkRoll(label, total)}>Roll</button></div>})}</section></div>
...
function IdentityChoice({ label, value, options, onChange }) { const existing = options.includes(value); return <label className="field identity-choice"><span>{label}</span><select value={existing ? value : ''} onChange={e => onChange(e.target.value)}><option value="">Custom {label.toLowerCase()}</option>{options.map(option => <option value={option} key={option}>{option}</option>)}</select>{!existing && <input aria-label={`Custom ${label}`} value={value} placeholder={`Enter custom ${label.toLowerCase()}`} onChange={e => onChange(e.target.value)}/>}</label> }
...
function SectionTitle({ icon, title, subtitle }) { const reminders = { 'Combat Summary': 'Move 30 feet each turn, even if you attack. Take one reaction per round. Free actions: talk, draw a weapon, or step 5 feet.', Attack: 'One Skill is used for both melee and ranged attacks. Talents can add damage.', Skills: `${subtitle} You can activate one Skill per turn.`, Talents: 'You can activate two Talents per turn. Sustained combat Talents occupy your available Talent Slots.', Weapons: 'You can attack once each turn, or move an extra 30 feet instead.' }; const note = reminders[title] || subtitle; return <div className="section-title"><h2><span>{icon}</span>{title}</h2>{note && <p>{note}</p>}</div> }
function Vital({ label, value, max, editable, onChange, roll }) { return <div className="vital"><span>{label}</span>{editable ? <input type="number" value={value} onChange={e=>onChange(e.target.value)}/> : <strong>{value}</strong>}{max !== undefined && <small>/ {max} max</small>}{roll && <button className="roll-button" onClick={roll}>Roll</button>}</div> }
