...
function Field({ label, onChange, wide, help = '', href = '', ...props }) { return <label className={`field ${wide ? 'wide' : ''}`}><span className="field-label">{href ? <a className={`sheet-reference-link ${help ? 'identity-label-tooltip' : ''}`} data-tooltip={help} href={href}>{label}</a> : label}</span><input {...props} onChange={e => onChange(e.target.value)}/></label> }
...
function SectionTitle({ title, subtitle }) {
  ...
  const reminders = { 'Combat Summary': 'Move 30 feet each turn, even if you attack. Take one reaction per round. Free actions: talk, draw a weapon, or step 5 feet.', Attack: 'One Skill is used for both melee and ranged attacks.', Skills: 'You can activate one Skill per turn.', Contacts: <strong>{subtitle || 'You begin with 3 + Charisma Contacts.'}</strong>, Talents: <strong>You can activate two Talents per turn. Sustained combat Talents occupy Combat Slots: one at level 0, plus one at levels 4 and 7.</strong>, 'Items & Traits': <><strong>Items explain why your Stats and Skills look the way they do.</strong>{' '}<strong>Traits describe your Hero’s personality, beliefs, habits, and complications.</strong></>, Weapons: 'You can have as many weapons as you like. You can attack once each turn, or move an extra 30 feet instead. Improvised ranged weapon attacks deal 1d4−1.' }
