...
function CharacterSheet() {
  ...
  return <div className="sheet-page">
    ...
    <div className="sheet-columns"><section className="sheet-section"><SectionTitle icon="▥" title="Stats" subtitle="Starting array: +3, +2, +1, 0, 0, −1. Each choice can only be used once, except 0 twice."/><div className="stat-list">{stats.map(([key, label, short, Icon]) => <div className="stat-row" key={key}><div className="stat-name"><Icon/><strong>{label} <span>({short})</span></strong></div><ScoreControl label={`${label} score`} value={character.stats[key]} options={[-1, 0, 1, 2, 3]} isOptionDisabled={option => statOptionUnavailable(key, option)} onChange={v => update(['stats', key], v)}/><button className="roll-button" onClick={() => checkRoll(label, character.stats[key])}>Roll</button></div>)}</div></section>
      <section className="sheet-section skills"><SectionTitle icon="★" title="Skills" subtitle="Starting array: +2, +2, +1, +1, +1, 0, 0, 0, −1"/><div className="skill-head"><span>Skill (Governing Stat)</span><span>Stat Score</span><span>Ability</span><span>Mod</span><span>Buff</span><span>Debuff</span><span>Total</span><span></span></div>{skillDefs.map(([key, label, defaultStat, Icon]) => { const total = skillTotal(key, defaultStat); const statName = stats.find(([statKey]) => statKey === defaultStat)?.[1]; const statScore = character.stats[defaultStat]; return <div className="skill-row" key={key}><strong className="skill-name"><Icon/><span>{label} <small>({statName})</small></span></strong><output className="skill-stat">{signed(statScore)}</output><ScoreControl label={`${label} ability`} value={character.skills[key].ability} options={[-1, 0, 1, 2]} onChange={v => update(['skills', key, 'ability'], v)}/>{['modifier','buffs','debuffs'].map(field => <NumberInput key={field} value={character.skills[key][field]} onChange={v => update(['skills', key, field], field === 'debuffs' ? -Math.abs(number(v)) : v)}/>)}<output>{signed(total)}</output><button className="roll-button" onClick={() => checkRoll(label, total)}>Roll</button></div>})}</section></div>
